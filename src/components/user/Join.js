...
const Join = () => {
  ...
  // useCallback : 함수의 메모이제이션을 위한 훅. (함수의 선언을 기억했따가 재사용하기 위한 훅)
  // 상태값 변경에 의해 화면에 재 렌더링이 발생할 때, 컴포넌트의 함수들도 재 선언이 됩니다.
  // useCallback으로 함수를 감싸주시면 이전에 생성된 함수를 기억했따가 재 사용하도록 하기 때문에
  ...
  // 만약 함수의 선언이 특정 상태가 변할 때 재선언 되어야 한다면, 의존성 배열에 상태변수를 선언하시면 됩니다.
  ...
  const fetchSignUpPost = async () => {
    /*
      ...
      Blob은 이미지, 사운드, 비디오 같은 멀티미디어 파일을 바이트 단위로 쪼개어 파일 손상을 
      방지하게 해 주는 타입. -> multipart/form-data에도 허용됨.
    */
